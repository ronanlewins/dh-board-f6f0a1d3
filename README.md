...
- `data/funnel.json` — the FULL three-block Meta funnel stage register (23 records: source, time/attribution basis, denominator, confidence tier, band source, sample size, interval, rendering prohibitions, caveat — schema in `reference/funnel-data-contract.md` in the workspace repo). **Gitignored**, same reason as `weeks.json` — it carries real business figures (impressions, clicks, enquiries, trials, members) and this repo is public. Hand-maintained; update it when a stage is added/removed/re-based or a pending value goes live. Never hand-edit the shipped payload — `build.sh` derives that.
- `validate.py` — the **schema validator** for `weeks.json`. Fails loudly (nonzero exit) and names the offending week/field if `weeks.json` is missing any required numeric, card/band field, or has a malformed `cpl_bands`. `build.sh` runs it FIRST, so a bad schema never deploys.
- `validate_funnel.py` — the **schema validator** for `funnel.json`. Enforces every required field on the full record (§3 of the funnel data contract) — a missing stage value, source, basis, sample size, band or caveat fails the build rather than shipping a blank stage. Also checks cross-field rules (band names only at tier "banded", `NEVER_COMPUTE`/cross-check guards present and correctly flagged, `denominator`/`to_node` referential integrity).
- `build.sh` — the build contract (see [Build contract](#build-contract)). Validates both schemas, inlines `data/weeks.json` **plus a compacted, build-time-ranked subset of `data/funnel.json`** into the template (→ `_build/index.html`), runs pre-encryption checks, encrypts with StatiCrypt using the password from `.staticrypt-pw`, then enforces a post-encryption size gate → produces the served `index.html`.
...
1. **Validate** — `python3 validate.py` against `data/weeks.json` (required numerics, card/band fields, band arrays), then `python3 validate_funnel.py` against `data/funnel.json` (every field in §3 of the funnel data contract, plus cross-field rules). Either failing aborts before anything renders.
2. **Inline + render** — inline `data/weeks.json` into `_src/template.html`, PLUS a compacted, build-time-ranked subset of `data/funnel.json` under a new top-level `funnel` key. The compaction step computes the §6 bottleneck ranking (largest loss share over `role:"step"` edges, Block 1 excluded, never across a bridge, pending/partial endpoints excluded) and flags the winner; it ships only `id/label/value/render/tier/band_label/caveat` per stage — the full record (source, basis, sample size, interval) stays in `data/funnel.json` and is never sent to the browser. Bridge records (`bridge1`/`bridge2`) are never shipped either — their annotation text is generated live in the browser from the linked node values, so shipping the record itself would only cost bytes. Output → `_build/index.html`.
...
`publish.sh` calls `build.sh` and pushes **only if it exited 0** — a validator/check/size failure blocks the deploy. The threshold ceiling (200KB) leaves room for the interactive JS plus years of weekly data — **but that headroom is thin.** As of the funnel build (05.08.26) the encrypted page sits around 202KB with ~2.8KB of margin, and one typical weekly entry (headline + working/not-working/next-move prose) adds ~8-9KB encrypted on its own. The gate is not to be raised (Conor reads this on a phone) — the next lever is pruning or archiving older weeks out of the live `weeks.json`, not growing the file indefinitely.

### Funnel ranking + rendering rules (do not weaken these to save bytes)

- `bridge1` (paid clicks → all traffic) and `bridge2` (all traffic → ads-attributed) never carry a percentage and are generated live from the linked node values, never hardcoded.
- `b2.e_reached_to_started` never renders as a percentage — mismatched populations (Clarity consent-accepters vs an all-traffic server count).
- `b3.e_attended_to_member` is `NEVER_COMPUTE` and is excluded from compaction (`role == "prohibited"`) — it must never be computed, not even internally.
- A pending or partial value never renders as `0` (`NO_ZERO_FILL`) — `fVal()` in the template always says "not yet measured" / "partial — N of 28 days" instead.
- Colour/band names appear only at tier `banded`. Directional and baseline-pending stages show a glyph + word (● / ◐ / ○), never a colour wash.
- If the byte budget is ever tight again, drop fields from the compacted funnel payload (or move detail to the analyst doc, per plan task 9e) — never weaken `validate_funnel.py`.
